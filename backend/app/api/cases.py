...
from app.models.database_models import ReturnCase, CaseImage, CVInspection, DefectFinding, AIReview, ReviewerDecision, CaseStatus
...
async def upload_image(
    ...
    db.add(row)
    case.status = CaseStatus.READY_FOR_INSPECTION.value
    db.flush()
...
def inspect_case(case_id: str, db: Session = Depends(get_db)):
    case = get_case_or_404(db, case_id)
    if not case.images:
        raise HTTPException(400, "Upload at least one image before inspection")
    case.status = CaseStatus.PROCESSING_CV.value
    db.commit()
    try:
        ...
        case.status = CaseStatus.CV_COMPLETE.value
...
def create_ai_review(case_id: str, db: Session = Depends(get_db)):
    case = get_case_or_404(db, case_id)
    case.status = CaseStatus.GENERATING_REVIEW.value
    ...
    db.add(row)
    case.status = CaseStatus.READY_FOR_REVIEW.value
...
def record_decision(case_id: str, payload: ReviewerDecisionCreate, db: Session = Depends(get_db)):
    ...
    case.status = mapping[payload.action]
    record_event(db, case.id, "REVIEWER_DECISION", {"action": payload.action})
    db.commit()
